...
Board.prototype.createGrid = function() {
    ...
    const start_x = parseInt(urlParams.get('sx'))
    const start_y = parseInt(urlParams.get('sy'))
    const end_x = parseInt(urlParams.get('ex'))
    const end_y = parseInt(urlParams.get('ey'))
    ...
    for (let c = 0; c < board.width; c++) {
      ...
      currentElement.onmousedown = (e) => {
        ...
        if (this.buttonsOn) {
          ...
          if (currentNode.status === "target" || currentNode.status === "object") {
...
Board.prototype.toggleButtons = function() {
  ...
  if (!this.buttonsOn) {
    ...
    document.getElementById("startButtonClearBoard").onclick = () => {
      ...
      const urlParams = new URLSearchParams(queryString);
      console.log(urlParams)
      const start_x = urlParams.get('sx')
      const start_y = urlParams.get('sy')
      const end_x = urlParams.get('ex')
      const end_y = parseInt(urlParams.get('ey')).toString()
    
      let start = start_y + "-" + start_x;
      let target = end_y + "-" + end_x;
        Object.keys(this.nodes).forEach(id => {
          let currentNode = this.nodes[id];
          let currentHTMLNode = document.getElementById(id);
          if (id === start) {
...
const urlParams = new URLSearchParams(queryString);
console.log(urlParams)
let height = parseInt(urlParams.get('h'))
let width = parseInt(urlParams.get('w'))
